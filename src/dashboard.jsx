...
// ─────────────────────────────────────────────────────────────────────────────
// LIVE DATA HOOK
// Fetches real stats from /api/[platform]. Falls back to DUMMY silently.
// ─────────────────────────────────────────────────────────────────────────────
  ...
  useEffect(() => {
    PLATFORMS.filter(p => !p.manual).forEach(p => {
      fetch(`/api/${p.id}`)
        ...
        .then(d => {
          if (d && !d.error) {
            setLiveData(prev  => ({ ...prev,  [p.id]: d    }));
            setAvailable(prev => ({ ...prev,  [p.id]: true }));
          }
        })
        .catch(() => {});
    });
  ...
  return (
    <div className="space-y-4">
        ...
        <div className="h-[420px] overflow-y-auto p-5 space-y-4">
          {messages.map((msg,i)=>(
            <div key={i} className={`flex ${msg.role==="user"?"justify-end":"justify-start"}`}>
              {msg.role==="assistant" && <div className="w-7 h-7 rounded-lg bg-slate-800 flex items-center justify-center text-sm mr-2 flex-shrink-0 mt-0.5">🤖</div>}
      ...
      <div className="bg-slate-50 border border-slate-100 rounded-xl p-4">
        ...
        <p className="text-xs text-slate-500 leading-relaxed">The agent has access to your full CMS ({cmsItems.length} pieces) including status, platforms, and tags.</p>
    ...
    <div className="space-y-6">
      ...
      {!isLive && (
        <div className="bg-slate-50 border border-dashed border-slate-200 rounded-xl p-4 flex items-center gap-3">
          ...
          <p className="text-sm text-slate-500">Showing <strong>demo data</strong> for {plat?.label}. Add the API key in Vercel env vars to see live stats.</p>
        </div>
      )}
      {isLive && (
        <div className="bg-emerald-50 border border-emerald-200 rounded-xl p-4 flex items-center gap-3">
          <span className="text-xl">{plat?.icon}</span>
          <p className="text-sm text-emerald-700">Showing <strong>live data</strong> for {plat?.label}.</p>
        </div>
      )}
...
function CMS() {
  ...
  function togglePlatform(platId)   {
    setNewItem(prev=>({ ...prev, platforms: prev.platforms.includes(platId) ? prev.platforms.filter(p=>p!==platId) : [...prev.platforms, platId] }));
  }
                ...
                {colItems.map(item=>(
                  <div key={item.id} className="bg-white rounded-lg border border-slate-100 p-3 shadow-sm hover:shadow-md transition-shadow cursor-default">
                    ...
                    <div className="flex flex-wrap gap-1 mb-2">
                      {item.platforms.map(pid=>{const pl=getPl(pid);return <span key={pid} className="text-[10px] px-1.5 py-0.5 rounded-full font-medium" style={{background:pl?.color+"18",color:pl?.color}}>{pl?.icon} {pl?.label}</span>;})}
                    </div>
            ...
            <tbody>
              {filtered.map((item,i)=>(
                <tr key={item.id} className={`border-b border-slate-50 hover:bg-slate-50 transition-colors ${i%2===0?"bg-white":"bg-slate-50/30"}`}>
                  <td className="px-4 py-3"><div><p className="font-medium text-slate-800 text-xs">{item.title}</p>{item.notes&&<p className="text-[11px] text-slate-400 mt-0.5 truncate max-w-[200px]">{item.notes}</p>}</div></td>
                  <td className="px-3 py-3 hidden md:table-cell"><span className="text-xs text-slate-500 bg-slate-100 px-2 py-0.5 rounded-full">{item.type}</span></td>
                  <td className="px-3 py-3">
                    <select value={item.status} onChange={e=>updateStatus(item.id,e.target.value)}
                      className="text-xs border border-slate-200 rounded-lg px-2 py-1 outline-none cursor-pointer bg-white font-medium" style={{color:getSt(item.status)?.color}}>
                      {CMS_STATUSES.map(s=><option key={s.id} value={s.id}>{s.icon} {s.label}</option>)}
                    </select>
                  </td>
                  <td className="px-3 py-3 hidden lg:table-cell"><div className="flex flex-wrap gap-1">{item.platforms.map(pid=>{const pl=getPl(pid);return <span key={pid} className="text-[10px] px-1 rounded" style={{background:pl?.color+"14",color:pl?.color}}>{pl?.icon}</span>;})}</div></td>
                  ...
                  <td className="px-3 py-3"><button onClick={()=>deleteItem(item.id)} className="text-xs text-slate-300 hover:text-rose-400 transition-colors">✕</button></td>
                </tr>
              ))}
            </tbody>
          </table>
...
function Scheduler() {
  ...
  function add() {
    if (!np.title||!np.date||!np.time) return;
    setPosts(p=>[...p,{...np,id:Date.now(),status:"draft"}]);
    setNp({title:"",platform:"instagram",date:"",time:"",type:"Post"});
    setShowForm(false);
  }
  ...
  return (
    <div className="space-y-5">
      <div className="flex flex-wrap items-center gap-3">
        <div className="flex gap-1 bg-slate-100 rounded-lg p-1">
          {["all","scheduled","review","draft"].map(f=>(
            <button key={f} onClick={()=>setFilter(f)} className={`px-3 py-1 rounded-md text-xs font-medium transition-all ${filter===f?"bg-white shadow-sm text-slate-800":"text-slate-500 hover:text-slate-700"}`}>
      ...
      {showForm && (
        <div className="bg-white rounded-xl border border-slate-200 p-5 shadow-sm space-y-3">
          ...
          <input value={np.title} onChange={e=>setNp(p=>({...p,title:e.target.value}))} placeholder="Post title" className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm outline-none focus:border-slate-400"/>
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            <select value={np.platform} onChange={e=>setNp(p=>({...p,platform:e.target.value}))} className="border border-slate-200 rounded-lg px-3 py-2 text-sm outline-none">
              {PLATFORMS.map(p=><option key={p.id} value={p.id}>{p.label}</option>)}
            </select>
            <select value={np.type} onChange={e=>setNp(p=>({...p,type:e.target.value}))} className="border border-slate-200 rounded-lg px-3 py-2 text-sm outline-none">
              {["Post","Video","Story","Reel","Thread","Article","Carousel","Update"].map(t=><option key={t}>{t}</option>)}
            </select>
            <input type="date" value={np.date} onChange={e=>setNp(p=>({...p,date:e.target.value}))} className="border border-slate-200 rounded-lg px-3 py-2 text-sm outline-none"/>
            <input type="time" value={np.time} onChange={e=>setNp(p=>({...p,time:e.target.value}))} className="border border-slate-200 rounded-lg px-3 py-2 text-sm outline-none"/>
          </div>
          <div className="flex gap-2">
            <button onClick={add} className="px-4 py-1.5 bg-slate-800 text-white text-xs font-medium rounded-lg">Add</button>
            <button onClick={()=>setShowForm(false)} className="px-4 py-1.5 text-slate-500 text-xs font-medium rounded-lg">Cancel</button>
          </div>
        </div>
      )}
      ...
      <div className="space-y-2">
        {filtered.map(post=>{
          const pl=getPl(post.platform);
          return (
            <div key={post.id} className="bg-white rounded-xl border border-slate-100 shadow-sm px-4 py-3 flex items-center gap-4 hover:border-slate-200 transition-colors">
              <div className="w-8 h-8 rounded-lg flex items-center justify-center text-lg flex-shrink-0" style={{background:pl?.color+"18"}}>{pl?.icon}</div>
              <div className="flex-1 min-w-0">
                <p className="text-sm font-medium text-slate-800 truncate">{post.title}</p>
                <p className="text-xs text-slate-400">{pl?.label} · {post.type}</p>
              </div>
              <div className="text-right flex-shrink-0">
                <p className="text-xs font-medium text-slate-700">{post.date}</p>
                <p className="text-xs text-slate-400">{post.time}</p>
              </div>
              <ScBadge status={post.status}/>
            </div>
          );
        })}
      </div>
  ...
  return (
    <div className="space-y-5">
      <div className="grid grid-cols-3 gap-3">
        {[{l:"Total",v:tasks.length,i:"📋"},{l:"Done",v:tasks.filter(t=>t.done).length,i:"✅"},{l:"Remaining",v:tasks.filter(t=>!t.done).length,i:"⏳"}].map(s=>(
      ...
      <div className="bg-white rounded-xl border border-slate-100 shadow-sm p-5">
        <p className="text-sm font-semibold text-slate-700 mb-4">Task Board</p>
        <div className="flex gap-2 mb-4">
          <input value={nt} onChange={e=>setNt(e.target.value)}
            onKeyDown={e=>e.key==="Enter"&&nt.trim()&&(setTasks(p=>[...p,{id:Date.now(),text:nt,assignee:"You",due:"TBD",priority:"med",done:false}]),setNt(""))}
            placeholder="Add a task…" className="flex-1 border border-slate-200 rounded-lg px-3 py-2 text-sm outline-none focus:border-slate-400"/>
        </div>
        <div className="space-y-2">
          {tasks.map(t=>(
            <div key={t.id} className={`flex items-center gap-3 p-3 rounded-lg border transition-all ${t.done?"bg-slate-50 border-slate-100 opacity-60":"bg-white border-slate-100 hover:border-slate-200"}`}>
              <input type="checkbox" checked={t.done} onChange={()=>setTasks(p=>p.map(x=>x.id===t.id?{...x,done:!x.done}:x))} className="w-4 h-4 accent-slate-700 cursor-pointer flex-shrink-0"/>
              <p className={`text-sm flex-1 ${t.done?"line-through text-slate-400":"text-slate-700"}`}>{t.text}</p>
              <span className={`text-xs font-medium px-2 py-0.5 rounded-full ${pc[t.priority]}`}>{t.priority}</span>
        ...
        <div className="p-5 space-y-3 h-72 overflow-y-auto">
          {msgs.map(m=>(
            <div key={m.id} className={`flex ${m.from==="You"?"justify-end":"justify-start"}`}>
              <div className={`max-w-xs px-4 py-2.5 rounded-2xl text-sm ${m.from==="You"?"bg-slate-800 text-white rounded-br-sm":"bg-slate-100 text-slate-800 rounded-bl-sm"}`}>
                {m.text}<p className="text-xs mt-1 opacity-40">{m.time}</p>
              </div>
            </div>
          ))}
        </div>
        <div className="border-t border-slate-100 p-3 flex gap-2">
          <input value={inp} onChange={e=>setInp(e.target.value)} onKeyDown={e=>e.key==="Enter"&&send()} placeholder="Message Sarah…" className="flex-1 bg-slate-50 border border-slate-200 rounded-lg px-3 py-2 text-sm outline-none focus:border-slate-400"/>
          <button onClick={send} className="px-4 py-2 bg-slate-800 text-white text-xs font-medium rounded-lg hover:bg-slate-700 transition-colors">Send</button>
        </div>
      </div>
    </div>
  );
...
function Community() {
  ...
  return (
    <div className="space-y-5">
      <div className="bg-white rounded-xl border border-slate-100 shadow-sm p-5">
        ...
        <div className="flex items-center gap-8 flex-wrap">
          <ResponsiveContainer width={140} height={140}>
            <PieChart><Pie data={SENTIMENT} dataKey="value" cx="50%" cy="50%" innerRadius={40} outerRadius={65} paddingAngle={3}>{SENTIMENT.map((s,i)=><Cell key={i} fill={s.color}/>)}</Pie><Tooltip formatter={v=>`${v}%`} contentStyle={{borderRadius:10,fontSize:12}}/></PieChart>
          </ResponsiveContainer>
          <div className="space-y-2">
            {SENTIMENT.map(s=><div key={s.name} className="flex items-center gap-2"><div className="w-2.5 h-2.5 rounded-full" style={{background:s.color}}/><span className="text-sm text-slate-600">{s.name}</span><span className="ml-6 text-sm font-bold text-slate-800">{s.value}%</span></div>)}
          </div>
        </div>
      </div>
      <div className="flex gap-1 bg-slate-100 rounded-lg p-1 w-fit">
        {["all","positive","neutral","negative"].map(f=>(
          <button key={f} onClick={()=>setFilter(f)} className={`px-3 py-1 rounded-md text-xs font-medium transition-all ${filter===f?"bg-white shadow-sm text-slate-800":"text-slate-500"}`}>
            {f[0].toUpperCase()+f.slice(1)}
          </button>
        ))}
      </div>
      <div className="space-y-2">
        {filtered.map(c=>{
          const pl=getPl(c.platform);
          return (
            <div key={c.id} className="bg-white rounded-xl border border-slate-100 shadow-sm p-4 flex gap-3 hover:border-slate-200 transition-colors">
              <div className="w-8 h-8 rounded-full flex items-center justify-center text-base flex-shrink-0" style={{background:pl?.color+"18"}}>{pl?.icon}</div>
              <div className="flex-1 min-w-0">
                <div className="flex items-center gap-2 mb-1 flex-wrap">
                  ...
                  <span className="text-xs text-slate-300">·</span>
                  <p className="text-xs text-slate-400">{c.time} ago</p>
                  <span className={`ml-auto text-xs font-medium px-2 py-0.5 rounded-full border ${ss[c.sentiment]}`}>{c.sentiment}</span>
                </div>
                <p className="text-sm text-slate-600">{c.text}</p>
                <div className="flex gap-3 mt-2">
                  <span className="text-xs text-slate-400">♥ {c.likes}</span>
                  <button className="text-xs text-slate-400 hover:text-slate-700">Reply</button>
                  <button className="text-xs text-slate-400 hover:text-slate-700">Flag</button>
                </div>
              </div>
            </div>
          );
...
function Connect({ available, gfm, onGfm }) {
  ...
  const [editGfm,  setEditGfm]  = useState(false);
  const [gfForm,   setGfForm]   = useState(gfm);
  ...
  return (
    <div className="space-y-4">
      <div className="bg-blue-50 border border-blue-200 rounded-xl p-4 flex gap-3">
        ...
        <div>
          <p className="text-sm font-semibold text-blue-800">Server-side API keys</p>
          <p className="text-xs text-blue-700 mt-0.5">All platform credentials live in Vercel environment variables — your keys never reach the browser. Add them in your Vercel project → Settings → Environment Variables, then redeploy.</p>
        </div>
      </div>

      {PLATFORMS.map(p => {
        const isLive  = available[p.id];
        const guide   = SETUP_GUIDES[p.id];
          ...
          <div key={p.id} className="bg-white rounded-xl border border-slate-100 shadow-sm p-4">
            <div className="flex items-center gap-3">
              ...
              <div className="flex-1">
                ...
                {p.manual
                  ? <p className="text-xs text-slate-400 mt-0.5">Manual data entry</p>
                  : isLive
                    ? <p className="text-xs text-emerald-600 font-medium mt-0.5">✓ Live data active</p>
                    : <p className="text-xs text-slate-400 mt-0.5">No API key detected — showing demo data</p>
                }
              </div>
              {p.manual ? (
                <button onClick={()=>setEditGfm(!editGfm)} className="px-3 py-1.5 text-xs font-medium rounded-lg border border-slate-200 text-slate-600 hover:bg-slate-50">
                  {editGfm?"Cancel":"Edit Data"}
                </button>
              ) : isLive ? (
                <span className="flex items-center gap-1.5 text-xs font-medium px-3 py-1.5 rounded-lg bg-emerald-50 text-emerald-700 border border-emerald-200">
                  <span className="w-1.5 h-1.5 rounded-full bg-emerald-500 animate-pulse"/>Live
                </span>
              ) : (
                <button onClick={()=>setExpanded(isOpen?null:p.id)} className="px-3 py-1.5 text-xs font-medium rounded-lg text-white hover:opacity-90 transition-opacity" style={{background:p.color}}>
                  {isOpen?"Hide guide":"Setup →"}
                </button>
              )}
            </div>

            {/* GoFundMe manual edit form */}
            {p.manual && editGfm && (
              <div className="mt-4 pt-4 border-t border-slate-100 space-y-3">
                <div className="grid grid-cols-2 gap-3">
                  {[{k:"title",l:"Title",t:"text"},{k:"url",l:"URL",t:"url"},{k:"goal",l:"Goal ($)",t:"number"},{k:"raised",l:"Raised ($)",t:"number"},{k:"donors",l:"Donors",t:"number"},{k:"daysLeft",l:"Days Left",t:"number"}].map(f=>(
                    <div key={f.k}>
                      <label className="text-xs text-slate-500 font-medium block mb-1">{f.l}</label>
                      <input type={f.t} value={gfForm[f.k]||""} onChange={e=>setGfForm(x=>({...x,[f.k]:f.t==="number"?Number(e.target.value):e.target.value}))}
                        className="w-full border border-slate-200 rounded-lg px-3 py-1.5 text-sm outline-none focus:border-slate-400"/>
                    </div>
                  ))}
                </div>
                <textarea rows={2} value={gfForm.latestUpdate||""} onChange={e=>setGfForm(x=>({...x,latestUpdate:e.target.value}))} placeholder="Latest update…"
            ...
            {!p.manual && isOpen && guide && (
              <div className="mt-4 pt-4 border-t border-slate-100">
                <p className="text-xs font-semibold text-slate-600 mb-2">Setup steps</p>
                <ol className="space-y-1.5">
                  {guide.steps.map((step,i)=>(
                    <li key={i} className="flex gap-2 text-xs text-slate-600">
                      <span className="w-4 h-4 rounded-full flex items-center justify-center text-[10px] font-bold text-white flex-shrink-0 mt-0.5" style={{background:p.color}}>{i+1}</span>
                      {step}
                    </li>
                  ))}
                </ol>
      ...
      <header className="bg-white border-b border-slate-100 sticky top-0 z-20">
        <div className="max-w-7xl mx-auto px-5 py-3 flex items-center gap-4">
          ...
          <div className="hidden lg:flex items-center gap-2 ml-3">
            <span className={`text-xs font-medium px-2.5 py-1 rounded-full border ${liveCount>0?"bg-emerald-50 text-emerald-700 border-emerald-200":"bg-slate-50 text-slate-500 border-slate-200"}`}>
          ...
          <div className="ml-auto flex items-center gap-3">
            {user && (
              <div className="flex items-center gap-2">
                <span className="text-xs text-slate-500 hidden md:block">{user.primaryEmailAddress?.emailAddress}</span>
                <button onClick={()=>signOut()} className="text-xs text-slate-400 hover:text-slate-700 transition-colors px-2 py-1 rounded-lg hover:bg-slate-100">Sign out</button>
              </div>
            )}
          </div>
        </div>
        <div className="max-w-7xl mx-auto px-5 flex gap-0 overflow-x-auto">
          {NAV.map(n=>(
            <button key={n.id} onClick={()=>setTab(n.id)}
              ...
              {n.icon} {n.label}
              {n.id==="connect"&&liveCount>0&&<span className="w-4 h-4 rounded-full bg-emerald-500 text-white text-[10px] font-bold flex items-center justify-center">{liveCount}</span>}
